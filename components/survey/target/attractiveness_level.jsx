...
export default function AttractivenessLevel({ data, setData, sub, setSub }) {
  const [value, setValue] = React.useState("");
  const [weight, setWeight] = useState(null);

  useEffect(() => {
    if (value == "") { setData({ ...data, attractiveness_level: null }); setSub(!sub); }
    else { setData({ ...data, attractiveness_level: value }); setSub(!sub); }
  }, [value]);
  ...
  const handleWeight = (newWeight) => {
    if (newWeight == weight) {
      ...
      setWeight(newWeight);
      setData({ ...data, attractiveness_level_w: newWeight });
    }
    setSub(!sub);
  };
  ...
  return (
    ...
    >
      <Typography className="heading2">애교 레밸</Typography>
      <Divider />
      <Typography className="basic-gray">
        원하는 상대방의 애교 레밸을 모두 선택해주세요.
      </Typography>
      ...
      <Container disableGutters sx={{display:'flex',justifyContent:'space-between', maxWidth:'280px', alignContent:'flex-end'}}>
        <Typography className="basic-gray">애교 많아요</Typography>
        <Typography className="basic-gray">무뚝뚝한 편이에요</Typography>
      </Container>
