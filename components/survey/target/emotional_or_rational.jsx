...
export default function EmotionalOrRational({ data, setData, sub, setSub }) {
  const [value, setValue] = React.useState("");
  const [weight, setWeight] = useState(null);

  useEffect(() => {
    if (value == "") { setData({ ...data, emotional_or_rational: null }); setSub(!sub); }
    else { setData({ ...data, emotional_or_rational: value }); setSub(!sub); }
  }, [value]);
  ...
  const handleWeight = (newWeight) => {
    if (newWeight == weight) {
      ...
      setWeight(newWeight);
      setData({ ...data, emotional_or_rational_w: newWeight });
    }
    setSub(!sub);
  };
  ...
  return (
    ...
    >
      <Typography className="heading2">감성/이성</Typography>
      <Divider />
      <Typography className="basic-gray">
        원하는 상대방의 감성/이성 정도를 모두 선택해주세요.
      </Typography>
      ...
      <Container disableGutters sx={{display:'flex',justifyContent:'space-between', maxWidth:'280px', alignContent:'flex-end'}}>
        <Typography className="basic-gray">매우 감성</Typography>
        <Typography className="basic-gray">매우 이성</Typography>
      </Container>
