...
export default function ExtrovertOrIntrovert({ data, setData, sub, setSub }) {
  const [value, setValue] = React.useState("");
  const [weight, setWeight] = useState(null);

  useEffect(() => {
    if (value == "") { setData({ ...data, extrovert_or_introvert: null }); setSub(!sub); }
    else { setData({ ...data, extrovert_or_introvert: value }); setSub(!sub); }
  }, [value]);
  ...
  const handleWeight = (newWeight) => {
    if (newWeight == weight) {
      ...
      setWeight(newWeight);
      setData({ ...data, extrovert_or_introvert_w: newWeight });
    }
    setSub(!sub);
  };
  ...
  return (
    ...
    >
      <Typography className="heading2">외향/내향</Typography>
      <Divider />
      <Typography className="basic-gray">
        원하는 상대방의 외향/내향 정도를 모두 선택해주세요.
      </Typography>
      ...
      <Container disableGutters sx={{display:'flex',justifyContent:'space-between', maxWidth:'280px', alignContent:'flex-end'}}>
        <Typography className="basic-gray">매우 외향</Typography>
        <Typography className="basic-gray">매우 내향</Typography>
      </Container>
