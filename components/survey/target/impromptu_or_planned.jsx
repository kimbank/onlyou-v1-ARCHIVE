...
export default function ImpromptuOrPlanned({ data, setData, sub, setSub }) {
  const [value, setValue] = React.useState("");
  const [weight, setWeight] = useState(null);

  useEffect(() => {
    if (value == "") { setData({ ...data, impromptu_or_planned: null }); setSub(!sub); }
    else { setData({ ...data, impromptu_or_planned: value }); setSub(!sub); }
  }, [value]);
  ...
  const handleWeight = (newWeight) => {
    if (newWeight == weight) {
      ...
      setWeight(newWeight);
      setData({ ...data, impromptu_or_planned_w: newWeight });
    }
    setSub(!sub);
  };
  ...
  return (
    ...
    >
      <Typography className="heading2">즉흥/계획</Typography>
      <Divider />
      <Typography className="basic-gray">
        원하는 상대방의 즉흥/계획 정도를 모두 선택해주세요.
      </Typography>
      ...
      <Container disableGutters sx={{display:'flex',justifyContent:'space-between', maxWidth:'280px', alignContent:'flex-end'}}>
        <Typography className="basic-gray">매우 즉흥</Typography>
        <Typography className="basic-gray">매우 계획</Typography>
      </Container>
