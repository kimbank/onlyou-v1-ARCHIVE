...
export default function JealousyLevel({ data, setData, sub, setSub }) {
  const [value, setValue] = React.useState("");
  const [weight, setWeight] = useState(null);

  useEffect(() => {
    if (value == "") { setData({ ...data, jealousy_level: null }); setSub(!sub); }
    else { setData({ ...data, jealousy_level: value }); setSub(!sub); }
  }, [value]);
  ...
  const handleWeight = (newWeight) => {
    if (newWeight == weight) {
      ...
      setWeight(newWeight);
      setData({ ...data, jealousy_level_w: newWeight });
    }
    setSub(!sub);
  };
