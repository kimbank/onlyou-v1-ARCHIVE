...
export default function PrefferedDating({ data, setData, sub, setSub }) {
  ...
  useEffect(() => {
    setData({ ...data, preffered_dating: value });
    setSub(!sub);
  }, [value]);
  ...
  const handleChange = (newValue) => {
    if (newValue == value) {
      setValue(null)
      setData({ ...data, preffered_dating: null });
    }
    else if (value == null) {
      setValue(newValue)
      setData({ ...data, preffered_dating: newValue });
    } else {
      setValue(newValue);
    }
    setSub(!sub);
  ...
  const handleWeight = (newWeight) => {
    if (newWeight == weight) {
      ...
      setWeight(newWeight);
      setData({ ...data,  preffered_dating_w: newWeight });
    }
    setSub(!sub);
  };
  ...
  return (
    ...
    >
      <Typography className="heading2">선호 데이트</Typography>
      <Divider />
      <Typography className="basic-gray">
        원하는 상대방의 선호 데이트를 선택해주세요.
      </Typography>
      ...
      >
        <Button variant="contained" sx={sx} onClick={() => handleChange(0)} color={value == 0 ? "primary" : "secondary"}>정적인 데이트 선호</Button>
