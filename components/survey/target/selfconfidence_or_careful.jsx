...
export default function SelfconfidenceOrCareful({ data, setData, sub, setSub }) {
  const [value, setValue] = React.useState("");
  const [weight, setWeight] = useState(null);

  useEffect(() => {
    if (value == "") { setData({ ...data, selfconfidence_or_careful: null }); setSub(!sub); }
    else { setData({ ...data, selfconfidence_or_careful: value }); setSub(!sub); }
  }, [value]);
  ...
  const handleWeight = (newWeight) => {
    if (newWeight == weight) {
      ...
      setWeight(newWeight);
      setData({ ...data, selfconfidence_or_careful_w: newWeight });
    }
    setSub(!sub);
  };
  ...
  return (
    ...
    >
      <Typography className="heading2">자기확신/신중</Typography>
      <Divider />
      <Typography className="basic-gray">
        원하는 상대방의 자기확신/신중 정도를 모두 선택해주세요.
      </Typography>
      ...
      <Container disableGutters sx={{display:'flex',justifyContent:'space-between', maxWidth:'280px', alignContent:'flex-end'}}>
        <Typography className="basic-gray">매우 자기확신</Typography>
        <Typography className="basic-gray">매우 신중</Typography>
      </Container>
