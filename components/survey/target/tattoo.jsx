...
export default function Tattoo({ data, setData, sub, setSub }) {
  const [value, setValue] = React.useState("");
  const [weight, setWeight] = useState(null);

  useEffect(() => {
    if (value == "") { setData({ ...data, tattoo: null }); setSub(!sub); }
    else { setData({ ...data, tattoo: value }); setSub(!sub); }
  }, [value]);
  ...
  const handleWeight = (newWeight) => {
    if (newWeight == weight) {
      ...
      setWeight(newWeight);
      setData({ ...data, tattoo_w: newWeight });
    }
    setSub(!sub);
  };
  ...
  return (
    ...
    >
        <Typography className="heading2">문신 유무</Typography>
        <Divider />
        <Typography className="basic-gray">
          원하는 상대의 문신 유무를 모두 선택해주세요.
        </Typography>
